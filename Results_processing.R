...
my_pal_scen<-c("brown2","forestgreen")
my_pal_scen_sens<-c("brown2","forestgreen", "cyan3", "darkorchid3")
my_pal_ssp<-c("forestgreen","dodgerblue3","darkgoldenrod3","firebrick3","black")

# Load the additional RCP results for sensitivity analysis
prj_4p5 <- loadProject("Results_4p5.dat")
prj_6p0 <- loadProject("Results_6p0.dat")
...
serv.sens <- getQuery(prj,"building service output by tech (new)" ) %>%
  bind_rows(getQuery(prj_4p5,"building service output by tech (new)")) %>%
  bind_rows(getQuery(prj_6p0,"building service output by tech (new)")) %>%
  mutate(sector = gsub("others","non-thermal",sector)) %>%
  filter(grepl("resid",sector)) %>%
  separate(sector,c("sector","decile"),sep = "_") %>%
  mutate(decile = gsub("d","D",decile)) %>%
  ...
  filter(year >= 2015, year <= 2050) %>%
  mutate(scenario = gsub("_2p6","-2p6",scenario),
         scenario = gsub("_4p5","-4p5",scenario),
         scenario = gsub("_6p0","-6p0",scenario)) %>%
  separate(scenario,c("model","narrative"),sep = "_") %>%
  mutate(scenario = if_else(grepl("2p6",narrative),"RCP2p6", "Baseline"),
         scenario = if_else(grepl("4p5",narrative),"RCP4p5", scenario),
         scenario = if_else(grepl("6p0",narrative),"RCP6p0", scenario)) %>%
  mutate(narrative = gsub("-2p6","",narrative),
         narrative = gsub("-4p5","",narrative),
         narrative = gsub("-6p0","",narrative)) %>%
  mutate(sector = gsub("resid ","",sector)) %>%
  separate(sector, c("sector","type"), sep = " ") %>%
  select(scenario,narrative,region,sector,type,subsector,technology,decile,year,value,Units) %>%
  left_join_error_no_match(pop, by = c("narrative", "region", "decile", "year")) %>%
...
serv.reg.fuel.agg.sens <- serv.sens %>%
  mutate(subsector = if_else(technology == "hydrogen", "hydrogen",subsector)) %>%
  group_by(scenario, region, narrative, sector, year, Units) %>%
  summarise(value = sum(value)) %>%
  ungroup() %>%
...
agg.serv2.sens<-serv.sens %>%
  select(-value_pc, -pop_thous,-unit_pc) %>%
  group_by(scenario,narrative,region,sector,decile,year,Units) %>%
  summarise(value = sum(value)) %>%
  ungroup() 

modern.palma.serv.rcp<- getQuery(prj,"building service output by tech (new)") %>%
  bind_rows(getQuery(prj_4p5,"building service output by tech (new)")) %>%
  bind_rows(getQuery(prj_6p0,"building service output by tech (new)")) %>%
  mutate(sector = gsub("others","non-thermal",sector)) %>%
  filter(grepl("resid",sector)) %>%
...
modern.palma.serv.plot.pre.rcp<-modern.palma.serv.rcp %>%
  mutate(scenario = paste0(scenario,"--2050")) %>%
  mutate(scenario = if_else(year == 2015, "2015",scenario))

modern.palma.serv.plot.rcp<-modern.palma.serv.plot.pre.rcp %>%
  filter(scenario != 2015) %>%
  filter(year == 2050) %>%
  bind_rows(modern.palma.serv.plot.pre.rcp %>% filter(scenario == 2015)) %>%
  select(-year) %>%
  mutate(scenario = as.character(scenario))

modern.palma.serv.plot.sens.rcp<-modern.palma.serv.plot.rcp %>%
  mutate(scenario = gsub("--2050","",scenario),
         scenario = paste0(scenario, "_" ,narrative),
         scenario = if_else(grepl("2015",scenario),"2015",scenario)) %>%
  distinct(scenario, subRegion, class, value) %>%
...
modern.palma.serv.plot.sens.rcp.fin <- slice(modern.palma.serv.plot.sens.rcp, 1:(nrow(modern.palma.serv.plot.sens.rcp)- 1))
  

modern.gini.serv.datapre.sens<- getQuery(prj,"building service output by tech (new)") %>%
  bind_rows(getQuery(prj_4p5,"building service output by tech (new)")) %>%
  bind_rows(getQuery(prj_6p0,"building service output by tech (new)")) %>%
  mutate(sector = gsub("others","non-thermal",sector)) %>%
  filter(grepl("resid",sector)) %>%
...
modern.gini.serv.rcp<-pridr::compute_gini_deciles(modern.gini.serv.datapre.sens %>% mutate(category = gsub("D", "d", category)) %>% mutate(Category = category),
                                              inc_col = "share" , 
                                              grouping_variables = c("scenario", "narrative", "region", "sector", "year")) %>%
  rename(gini = output_name) %>%
  select(scenario, narrative, region, sector, year, gini) %>%
...
modern.gini.serv.plot.pre.rcp<-modern.gini.serv.rcp %>%
  mutate(scenario = paste0(scenario,"--2050")) %>%
  mutate(scenario = if_else(year == 2015, "2015",scenario))

modern.gini.serv.plot.rcp<-modern.gini.serv.plot.pre.rcp %>%
  filter(scenario != 2015) %>%
  filter(year == 2050) %>%
  bind_rows(modern.gini.serv.plot.pre.rcp %>% filter(scenario == 2015)) %>%
  select(-year) %>%
  mutate(scenario = as.character(scenario))

modern.gini.serv.plot.sens.rcp<-modern.gini.serv.plot.rcp %>%
  mutate(scenario = gsub("--2050","",scenario),
         scenario = paste0(scenario, "_" ,narrative),
         scenario = if_else(grepl("2015",scenario),"2015",scenario)) %>%
  distinct(scenario, subRegion, class, value) %>%
...
modern.gini.serv.plot.sens.rcp.fin <- slice(modern.gini.serv.plot.sens.rcp, 1:(nrow(modern.gini.serv.plot.sens.rcp)- 1))
...
# Prepare data for sensitivity analysis:
nonco2.sens<-read.csv("./workflow/data/nonco2_sens.csv") %>%
  separate(scenario, c("scenario","x"), sep = ",") %>%
  select(-x) %>%
  gather(year, value, -scenario, -region, -sector, -ghg, -unit) %>%
  mutate(year = gsub("X","",year)) %>%
...
resid.nonco2.reg.sens<- nonco2.sens %>%
  filter(grepl("resid",sector)) %>%
  separate(sector, c("sector","decile"), sep = "_") %>%
  mutate(decile = gsub("d","D",decile)) %>%
  mutate(sector = gsub("resid ","",sector)) %>%
...
resid.nonco2_pc.sens<- nonco2.sens %>%
  filter(grepl("resid",sector)) %>%
  separate(sector, c("sector","decile"), sep = "_") %>%
  mutate(decile = gsub("d","D",decile)) %>%
  mutate(sector = gsub("resid ","",sector)) %>%
...
# Prepare data for sensitivity analysis:
price.sens<-getQuery(prj, "prices of all markets") %>%
  bind_rows(getQuery(prj_4p5, "prices of all markets")) %>%
  bind_rows(getQuery(prj_6p0, "prices of all markets")) %>%
  mutate(region = if_else(grepl("Africa_Eastern",market),"Africa_Eastern","a"),
         region = if_else(grepl("Africa_Northern",market),"Africa_Northern",region),
         region = if_else(grepl("Africa_Southern",market),"Africa_Southern",region),
         region = if_else(grepl("Africa_Western",market),"Africa_Western",region),
...
expen.sens<-getQuery(prj,"building final energy by service and fuel") %>%
  bind_rows(getQuery(prj_4p5,"building final energy by service and fuel")) %>%
  bind_rows(getQuery(prj_6p0,"building final energy by service and fuel")) %>%
  filter(grepl("resid",sector)) %>%
  separate(sector, c("sector","decile"), sep = "_") %>%
  mutate(decile = gsub("d","D",decile)) %>%
  filter(year >= 2015, year <= 2050) %>%
  ...
  ungroup() %>%
  rename(market = input) %>%
  left_join_error_no_match(price.sens, by = c("scenario", "narrative", "year","market","region")) %>%
  mutate(expen_dolar = value_ej * 1E9 * price * gdp_deflator(2015, 1975)) %>%
  group_by(scenario, narrative, region, decile, year) %>%
  summarise(expen_dolar = sum(expen_dolar)) %>%
  ungroup() %>%
  ...
  mutate(gdppc_dolar = value * 1E3,
         perc_exp_residEn = expen_pc / gdppc_dolar) %>%
  select(scenario, narrative, region, decile, year, perc_exp_residEn) %>%
  left_join_error_no_match(agg.serv2.sens %>%
                             group_by(scenario, region, decile, year, narrative) %>%
                             summarise(value = sum(value)) %>%
                             ungroup(),
                           by = c("scenario","region","decile","year","narrative")) %>%
...
plot_serv_reg_agg<- ggplot(serv.reg.fuel.agg.sens,
                           aes(x = year,
                               y = value,
                               color = scenario,
                               fill = scenario)) +
  ...
  scale_color_manual(values = my_pal_scen_sens) + 
  scale_fill_manual(values = my_pal_scen_sens) 
...
plot_flsp_agg<- ggplot(flsp_plot_agg %>% filter(scenario == "Baseline"),
                       aes(x = year,
                           y = flsp_bm2,
                           color = scenario,
                           fill = scenario)) +
...
#-------------------------------------------
#-------------------------------------------
# Service output demand by region, period, scenario, narrative, service, and fuel for RCP4.5 and RCP6.0
plot_tot_serv_fuel_4p5<- ggplot(serv.reg.fuel.sens %>% filter(scenario == "RCP4p5"),
                                 aes(x = year,
                                     y = value,
                                     color = subsector,
                                     fill = subsector)) +
  geom_line() +
  geom_ribbon(data = serv.reg.fuel.sens %>% filter(scenario == "RCP4p5"),
              aes(ymin=min, ymax=max),
              alpha=0.3, 
              linetype = 0) +
  facet_grid(sector~region, scales = "free") +
...
plot_tot_serv_fuel_4p5
ggsave("./figures/SI/AggServOutFuel_rcp4p5.tiff", plot_tot_serv_fuel_4p5, "tiff", dpi = 200)

plot_tot_serv_fuel_6p0<- ggplot(serv.reg.fuel.sens %>% filter(scenario == "RCP6p0"),
                                aes(x = year,
                                    y = value,
                                    color = subsector,
                                    fill = subsector)) +
  geom_line() +
  geom_ribbon(data = serv.reg.fuel.sens %>% filter(scenario == "RCP6p0"),
              aes(ymin=min, ymax=max),
              alpha=0.3, 
              linetype = 0) +
  facet_grid(sector~region, scales = "free") +
...
plot_tot_serv_fuel_6p0
ggsave("./figures/SI/AggServOutFuel_rcp6p0.tiff",plot_tot_serv_fuel_6p0,"tiff",dpi=200)

#--------------------------------------

# Service output by period, sector, scenario, and decile
plot_agg_serv_ssp_rcp<- ggplot(agg.serv.sens %>% filter(scenario == "RCP6p0"),
                               aes(x=year,
                                   y=value,
                                   color=factor(decile,levels = c("D1","D2","D3","D4","D5","D6","D7","D8","D9","D10")),
                                   fill=factor(decile,levels = c("D1","D2","D3","D4","D5","D6","D7","D8","D9","D10")))) +
...
ggsave("./figures/SM/ServOut_ssp_rcp6p0.tiff",plot_agg_serv_ssp_rcp,"tiff",dpi=200)

#-------------------------------------------
# Per capita service output in 2050 per scenario, region, service, decile, and fuel in the SSP2 narrative
PcservList<- serv.sens %>% filter(narrative == "SSP2")  %>%  mutate(subsector = if_else(technology == "hydrogen", "hydrogen",subsector))
...
#-------------------------------------------
# Palma ratio for alternative narratives --  RCPs
map_rcp<-rmap::map(data = modern.palma.serv.plot.sens.rcp.fin %>% 
                      rename(sector = class) %>%
                      filter(scenario %in% c("2015","RCP6p0") ) %>%
                      mutate(subRegion = if_else(subRegion == "EU-12","EU_12",subRegion),
                             subRegion = if_else(subRegion == "EU-15","EU_15",subRegion)) %>%
                      select(-scenario),
                    row = "sector",
...
ggplot2::ggsave("./figures/SM/Palma_map_2050_rcp6p0.tiff",map_rcp_fin,"tiff",dpi=200)
...
map_rcp<-rmap::map(data = modern.gini.serv.plot.sens.rcp.fin %>% 
                     rename(sector = class) %>%
                     filter(scenario %in% c("2015","RCP6p0") ) %>%
                     mutate(subRegion = if_else(subRegion == "EU-12","EU_12",subRegion),
                            subRegion = if_else(subRegion == "EU-15","EU_15",subRegion)) %>%
...
ggplot2::ggsave("./figures/SM/Gini_map_2050_rcp6p0.tiff",map_rcp_fin,"tiff", width = 10, height = 5)
...
plot_agg_nonco2<- ggplot(agg.nonco2.sens,
                         aes(x=year,
                             y=value,
                             color = scenario,
                             fill = scenario)) +
...
plot_agg_nonco2_resid<- ggplot(resid.nonco2.reg.sens,
                         aes(x=year,
                             y=value,
                             color = scenario,
                             fill = scenario)) +
...
plot_expen<- ggplot(expen.sens %>% filter(year %in% c(2050)),
                    aes(x = value_pc,
                        y = perc_exp_residEn,
                        color = factor(decile,levels = c("D1","D2","D3","D4","D5","D6","D7","D8","D9","D10")),
                        fill = scenario,
  ...
  scale_fill_manual(values = c("grey95","white", "gray65", "grey80")) + 
  scale_color_manual(values = my_pal) + 
